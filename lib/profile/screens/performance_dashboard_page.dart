...
class _PerformanceDashboardPageState extends State<PerformanceDashboardPage> {
  final PerformanceDashboardService _dashboardService = PerformanceDashboardService();
  final QuizStatsService _quizStatsService = QuizStatsService();

  Period _activePeriod = Period.thisWeek;
  DateTimeRange? _customDateRange;
  String _selectedCategory = 'All Subjects';
  ...
  StreamSubscription? _testAttemptsSubscription;
  ...
  void dispose() {
    ...
    _testAttemptsSubscription?.cancel();
    super.dispose();
  }

  void _setupSubscriptions(String uid) {
    _attemptsSubscription?.cancel();
    _sessionsSubscription?.cancel();
    _testAttemptsSubscription?.cancel();

    _attemptsSubscription = EduverseFirebase.firestore
        .collection('users')
        .doc(uid)
        ...
        .listen((_) {
      if (mounted) {
        _loadData(skipLoadingIndicator: true);
      }
    });

    _sessionsSubscription = EduverseFirebase.firestore
        .collection('users')
        .doc(uid)
    ...
    _testAttemptsSubscription = EduverseFirebase.firestore
        .collection('users')
        .doc(uid)
  ...
  Future<void> _loadData({bool skipLoadingIndicator = false}) async {
    if (!mounted) return;

    final user = EduverseFirebase.auth.currentUser;
    if (user != null && _attemptsSubscription == null) {
      _setupSubscriptions(user.uid);
    }
    ...
    try {
      ...
      final stats = await _dashboardService.getAggregatedStats(
        _activePeriod,
        customStart: start,
        customEnd: end,
      );
      final categories = await _quizStatsService.getDistinctCategories();

      final Map<String, String> fetchedTitles = {};
      final List<Future<void>> fetchFutures = [];
      final Set<String> processedIds = {};

      for (final attempt in stats.rawTestAttempts) {
      ...
      if (mounted) {
        setState(() {
          _errorMessage = 'Failed to load performance metrics';
          _isLoading = false;
        });
      }
    }
  ...
  void _onPeriodChanged(Period period) {
    if (_activePeriod == period && _customDateRange == null) return;
    setState(() {
  ...
  Future<void> _selectCustomDateRange() async {
    ...
    if (picked != null) {
      setState(() {
        _activePeriod = Period.thisWeek; // reuse thisWeek as custom trigger
  ...
  Widget _buildBody() {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return _buildDashboardContent(isWide);
      },
    );
  }
  ...
  Widget _buildDashboardContent(bool isWide) {
    ...
    final filteredAttempts = stats.rawQuizAttempts.where((attempt) {
      if (_selectedCategory == 'All Subjects') return true;
      return attempt['categoryLabel'] == _selectedCategory;
    }).toList();

    final filteredSessions = stats.rawWatchSessions.where((session) {
      if (_selectedCategory == 'All Subjects') return true;
      return session['subjectName'] == _selectedCategory;
    }).toList();

    // Line Chart Spots sorted chronologically
    ...
    for (final tsId in stats.purchasedTestSeriesIds) {
      totalTestSeriesTests += stats.testSeriesTestCounts[tsId] ?? 0;
    }

    int completedTestSeriesTests = 0;
    for (final attempt in stats.rawTestAttempts) {
      final attemptId = attempt['id'] as String;
      final belongsToEnrolled = stats.purchasedTestSeriesIds.any((tsId) => attemptId.startsWith(tsId));
                    ...
                    children: [
                      ...
                      DropdownButton<String>(
                        ...
                        items: [
                          ...
                          ..._categories.toSet().map((cat) => DropdownMenuItem(value: cat, child: Text(cat))),
                        ],
                        onChanged: (val) {
                          if (val != null) {
                            setState(() {
          ...
          isWide
              ? Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    Expanded(child: _buildStatCard('Study Time', stats.formattedStudyTime, 'Video Sessions', true, Colors.green)),
                    const SizedBox(width: 12),
              ...
              : Column(
                  children: [
                    ...
                    const SizedBox(height: 8),
                    _buildStatCard('Study Time', stats.formattedStudyTime, 'Video Sessions', true, Colors.green),
                    const SizedBox(height: 8),
  ...
  Widget _buildStatCard(String title, String value, String subtext, bool isPositive, Color accentColor) {
    ...
    final isNeutral = subtext == '0%' || subtext == '0 quizzes' || subtext == 'N/A' || subtext == 'Video Sessions' || subtext == 'Tests Completed';
    IconData trendIcon = Icons.trending_flat;
    Color trendColor = Colors.grey;

    if (!isNeutral) {
      trendIcon = isPositive ? Icons.trending_up : Icons.trending_down;
      trendColor = isPositive ? Colors.green.shade600 : Colors.red.shade600;
    }

    String periodText = 'from last week';
    if (_activePeriod == Period.thisMonth) {
      periodText = 'from last month';
    } else if (_activePeriod == Period.thisYear) {
          ...
          children: [
            ...
            SizedBox(
              ...
              child: hasData
                  ...
                  : const Center(
                      child: Text(
                        'No activity trend data available for this range',
  ...
  Widget _buildPieChartCard(AccuracyData accuracy) {
    final hasData = (accuracy.correct + accuracy.wrong + accuracy.unattempted) > 0;
  ...
  Widget _buildTestAttemptsTable(DashboardStats stats) {
    // Filter attempts to enrolled test series only
    final filteredTestAttempts = stats.rawTestAttempts.where((attempt) {
      final attemptId = attempt['id'] as String;
      return stats.purchasedTestSeriesIds.any((tsId) => attemptId.startsWith(tsId));
    }).toList();
