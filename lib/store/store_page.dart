    ...
    return StreamBuilder<List<CombinationPack>>(
      ...
      builder: (context, snapshot) {
        ...
        return GridView.builder(
          ...
          itemBuilder: (context, index) {
            final pack = packs[index];
            final discount = pack.realPrice > pack.finalPrice
        ...
        child: ListView(
          ...
          children: [
            ...
            if (pack.batches.isNotEmpty) ...[
              ...
              ...pack.batches.map((b) => FutureBuilder<DocumentSnapshot>(
                future: FirebaseFirestore.instance
                    .collection('courses')
                    .doc(b['courseId'])
                    .get(),
                builder: (context, snapshot) {
                  final courseTitle = snapshot.hasData && snapshot.data!.exists
                      ? (snapshot.data!.data() as Map<String, dynamic>)['title'] ?? 'Course'
                      : 'Loading Course...';
                  return Card(
                    elevation: 0,
            ...
            if (pack.testSeries.isNotEmpty) ...[
              ...
              ...pack.testSeries.map((tsId) => FutureBuilder<DocumentSnapshot>(
                future: FirebaseFirestore.instance
                    .collection('test_series')
                    .doc(tsId)
                    .get(),
                builder: (context, snapshot) {
                  final tsTitle = snapshot.hasData && snapshot.data!.exists
                      ? (snapshot.data!.data() as Map<String, dynamic>)['title'] ?? 'Test Series'
                      : 'Loading Test Series...';
                  return Card(
                    elevation: 0,
                    margin: const EdgeInsets.only(bottom: 8),
